...
export default function VotingPage({ params }: VotingPageProps) {
  ...
  const handleVote = async (registrationId: string) => {
    ...
    // Check if trying to vote for the same costume in the same category
    if (myVotes[selectedCategory] === registrationId) {
      const categoryLabel = CATEGORY_CONFIG[selectedCategory].label;
      setVotingError(
        `You've already voted for this costume in "${categoryLabel}". Click "Remove Vote" to unvote.`
      );
      return;
    }

    // Optimistic UI update
    const previousVote = myVotes[selectedCategory];
    ...
    try {
      ...
      setMyVotes((prev) => ({ ...prev, [selectedCategory]: previousVote }));
      setVotingError(
  ...
  const handleUnvote = async (category?: Category) => {
    if (!attendeeId) return;

    const categoryToRemove = category || selectedCategory;

    // Optimistic UI update
    const previousVote = myVotes[categoryToRemove];
    setMyVotes((prev) => ({ ...prev, [categoryToRemove]: null }));
    setVotingError(null);
    ...
    try {
      ...
      // Rollback optimistic update
      setMyVotes((prev) => ({ ...prev, [categoryToRemove]: previousVote }));
      setVotingError(
